...
// var mysql = MYSQL.createClient({user:'wap', password:'wap'});
// mysql.query("USE bridgegame;");
	...
	socket.on('login',function(data,callback)
	{
		...
		for(var _id in users)
		{
			if (!users[_id].to && _id != user.id
				&& users[_id].city == user.city
				&& ( users[_id].he == user.sex || users[_id].he == 'any')
				&& (users[_id].sex == user.he || user.he == 'any')
